...
class test_Deploy__Service__to__main(TestCase):
    @classmethod
    def setUpClass(cls):
        cls.deploy_fast_api__main  = Deploy__Service(stage = 'main')

        with cls.deploy_fast_api__main as _:
            if _.aws_config.aws_configured() is False:
                pytest.skip("this test needs valid AWS credentials")

    def test_1__check_stages(self):
        assert self.deploy_fast_api__main .stage == 'main'

    def test_2__upload_dependencies(self):
        upload_results = self.deploy_fast_api__main.upload_lambda_dependencies_to_s3()
        assert list_set(upload_results) == LAMBDA_DEPENDENCIES__FAST_API_SERVERLESS

    def test_3__create(self):
        assert self.deploy_fast_api__main .create() is True

    def test_4__invoke(self):
        assert self.deploy_fast_api__main .invoke().get('errorMessage') == DEFAULT__ERROR_MESSAGE__WHEN_FAST_API_IS_OK

    def test_4__invoke__function_url(self):
        version = {'version': version__mgraph_ai_service_github_digest}
        assert self.deploy_fast_api__main .invoke__function_url('/info/version') == version

    # def test_4__delete(self):
    #     assert self.deploy_fast_api__main .delete() is True