...
class GitHub__API(Type_Safe):
    ...
    @type_safe
    def repository__contents__as_strings(self, owner: Safe_Id, repo: Safe_Id, ref: Safe_Id = Safe_Id("main")):
        repo_files_contents = self.repository__contents__as_bytes(owner=owner, repo=repo, ref=ref)
        contents_as_strings = {}
        for file_name, file_contents in repo_files_contents.items():
            try:
                contents_as_strings[file_name] = file_contents.decode()                                                 # convert the file contents into a string
    ...
    def fix_repo_file_name(self, path: str) -> str:                               #  Removes the first path segment from a repo file path.   Skips directories (paths ending with '/').
        if '/' not in path:
            return None                                                     # no slashes, can't strip root folder
        if path.endswith('/'):
            return None                                                     # it's a directory, skip
        return path.partition('/')[2]                                       # get everything after first '/'
